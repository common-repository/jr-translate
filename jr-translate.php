<?php
/*
Plugin Name: JR Translate
Plugin URI: http://www.jakeruston.co.uk/2009/11/wordpress-plugin-jr-translate/
...
add_action('admin_menu', 'jr_translate_add_pages');
...
if (!function_exists("jr_translate_refresh")) {
function jr_translate_refresh() {
update_option("jr_submitted_translate", "0");
}
}

register_activation_hook(__FILE__,'translate_choice');

function translate_choice () {
if (get_option("jr_translate_links_choice")=="") {

if (_iscurlinstalled()) {
$pname="jr_translate";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_translate", "1");
wp_schedule_single_event(time()+172800, 'jr_translate_refresh'); 
} else {
...
update_option("jr_translate_links_choice", $content);
}
}

if (get_option("jr_translate_link_personal")=="") {
$content = curl_get_contents("http://www.jakeruston.co.uk/p_pluginslink4.php");

update_option("jr_translate_link_personal", $content);
}
}

// jr_translate_options_page() displays the page content for the Test Options submenu
function jr_translate_options_page() {
...
<?php
    ...
    echo "<h2>" . __( 'JR Translate Plugin Options', 'mt_trans_domain' ) . "</h2>";
$blog_url_feedback=get_bloginfo('url');
...
<input type="radio" name="<?php echo $data_field_name_5; ?>" value="No" <?php echo $change11; ?> id="Please do not disable plugin support - This is the only thing I get from creating this free plugin!" onClick="alert(id)">No
</p><hr />
...
$linkper=utf8_decode(get_option('jr_translate_link_personal'));

if (get_option("jr_translate_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_translate_links_choice'));
...
update_option("jr_translate_links_choice", $new);
update_option("jr_translate_link_newcheck", "444");
}

if (get_option("jr_submitted_translate")=="0") {
$pname="jr_translate";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_translate", "1");
...
wp_schedule_single_event(time()+172800, 'jr_translate_refresh'); 
} else if (get_option("jr_submitted_translate")=="") {
$pname="jr_translate";
$url=get_bloginfo('url');
$current=get_option("jr_translate_links_choice");
...
echo "<p style='font-size:x-small'>Translate Plugin created by ".$linkper." - ".stripslashes(get_option('jr_translate_links_choice'))."</p>";
}
...
register_sidebar_widget("JR Translate", "show_translate");
}
